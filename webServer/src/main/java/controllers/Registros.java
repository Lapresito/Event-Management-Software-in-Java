	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		try {
			...
			if (nickname == null || nickname.isEmpty()) {//quiero ver todos los registros
				...
				boolean acceso = registro.isAsistio() || usuario.getNickname().equals(nickname) || ("organizador".equalsIgnoreCase(tipo) && usuario.getNickname().equals(edicion.getOrganizador())) ;
				if (!acceso) {
					req.setAttribute("error", "Acceso No permitido.");
